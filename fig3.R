...
ambiente<-read.table("RedSea.Mangrove.csv",header=T,sep=",")

pippo<-as_datetime(ambiente$grouped_time, format = '%Y-%m-%d %H:%M') ##best conversion in datetime
head(pippo)
...
##Temperature Massime per i Tropical
...
setwd("~/Dropbox/Articoli/Perspective Frontiers/Hyperossia2/Analysis/")
letale<-read.table("Figure3/LT50_figure_2.txt",header=T,sep="\t")
...
struttura<-read.table("~/Dropbox/Articoli/Hyperossia2/Hyperoxia_V2/Plasticity_TABLE.txt",header=T,sep="\t")
str(struttura)


#Alta predicibilità --->  controllo dei safety margins   ----> più efficiente risposta termica
